...
def define_vggish_slim(training=False):
    ...
    ----------
    training
        If true, all parameters are marked trainable.
    ...
    with SLIM.arg_scope(
        ...
        features = tf.placeholder(
            tf.float32, shape=(None, params.NUM_FRAMES, params.NUM_BANDS), name="input_features"
        )
...
def load_vggish_slim_checkpoint(session, checkpoint_path):
    ...
    ----------
    session
        An active TensorFlow session.
    checkpoint_path
        Path to a file containing a checkpoint that is
